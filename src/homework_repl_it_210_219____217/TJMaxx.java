package homework_repl_it_210_219____217;
...
public class TJMaxx {
	...
	public List<String> getAllItemNames() {
		...
		for (int i = 0; i < onSaleItemsCount(); i++) {
			allList.add(onSaleItems.get(i).getName());
		}
	...
	public void buyItem(int catalogNumber) {
		...
		for (int i = 0; i < regularItemsCount(); i++) {
			if (regularItems.get(i).getCatalogNumber() ==catalogNumber ) {
				int q = regularItems.get(i).getQuantity()-1;
...
//					removeItem(regularItems.get(i).getCatalogNumber());
					...
					regularItems.get(i).setQuantity(q);
					break;
				}
		...
		for (int i = 0; i < onSaleItemsCount(); i++) {
			if (onSaleItems.get(i).getCatalogNumber() ==catalogNumber ) {
				int q = onSaleItems.get(i).getQuantity()-1;
				
				if (q == 0) {
					onSaleItems.remove(onSaleItems.get(i));
				//	removeItem(onSaleItems.get(i).getCatalogNumber());
				}else {
					onSaleItems.get(i).setQuantity(q);
					break;
				}
			}
